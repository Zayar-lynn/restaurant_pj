...
<?php 
	...
	if (isset($_POST['submit'])) {
		...
		foreach ($pat as $key) 
		{
			...
			//print_r($result);
			if (!empty($result)) {
			  $yes_query = "INSERT INTO good (r_id,title) VALUES ('$restaurant_id','$str')";
		...
		if (empty($result)) {
			$no_query = "INSERT INTO bad (r_id,title) VALUES ('$restaurant_id','$str')";
		  	mysqli_query($connection,$no_query);
		  	
		  	$sql= "UPDATE add_restaurant set point=point-0.5 WHERE r_id='$restaurant_id'";
		  	mysqli_query($connection,$sql);
		}
	}
...
<br><br><br><br><br>
	<div class="container">
		<div class="row">
			...
			s					</div><br><br>
						 <form action="" method="post">
								<textarea name="string" id="" placeholder="What are some ways> could improve?" cols="50" rows="7"></textarea>
							   </div>
							<div class="modal-footer">
								<div class="form-group">
									<button class="btn btn-default pull-left" name="submit">Share</button>
